        ...
        if (!result.error && result.response) {
            for (let i = 0; i < result.response.length; i++) {
                ...
                btnEdit.addEventListener('click', function () {
                    localStorage.setItem('editIncomeId', result.response[i].id);
                })
                ...
                this.cards.insertBefore(card, this.cardAddIncome);
            }
                ...
                delBtn.addEventListener('click', function () {
                    ...
                    if (id) {
                        agreeDelete.onclick = async function () {
                            const delResult = await HttpUtils.request('/categories/income/' + id, "DELETE", true)
